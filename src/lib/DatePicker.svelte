<script lang="ts">
	...
	let {
		...
		showYear,
		allowRange
	}: { start?: Date; end?: Date; showYear?: boolean; allowRange?: boolean } = $props();
					...
					<tr>
						{#each row as day}
							<td
								class="day"
								class:hoverable={day}
								class:current={day === selectedDay && selectedMonth === currMonth}
								class:selected={(day &&
									start &&
									new Date(selectedYear, selectedMonth, day).getTime() === start.getTime()) ||
									(day &&
										start &&
										end &&
